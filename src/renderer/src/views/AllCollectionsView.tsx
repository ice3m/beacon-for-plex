...
import { useAllCollections } from '../lib/hooks'
import { useNav } from '../state/nav'
import { SkeletonGrid } from '../components/Skeleton'
import { ErrorState } from '../components/States'

/** Server-wide collections, grouped by the library they belong to. */
export function AllCollectionsView({ serverId }: { serverId: string }): JSX.Element {
  const { data, isLoading, isError, error, refetch } = useAllCollections(serverId)
  const { navigate } = useNav()

  // Group by owning library (tagged into parentTitle by the backend).
  const groups = useMemo(() => {
    const map = new Map<string, PlexMediaItem[]>()
    for (const c of data ?? []) {
      const lib = c.parentTitle ?? 'Collections'
  ...
  return (
    <div className="py-6">
      <h1 className="mb-6 px-8 text-3xl font-bold">Collections</h1>
      ...
      {data && data.length === 0 && (
        <p className="px-8 text-ink-muted">No collections found on this server.</p>
      )}

      {groups.map(([library, cols]) => (
        <section key={library} className="mb-8">
          <h2 className="mb-3 px-8 text-lg font-semibold text-ink-muted">{library}</h2>
